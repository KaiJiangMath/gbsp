...
void fn_stu_bulk_param_input_init (stu_bulk_param	*sbulkparam)
{
	...
	sbulkparam->Four_num				= 8;
	sbulkparam->enlarge					= 10.0;
	sbulkparam->plot_num				= 32;

	/* the iteration parameters; */
	sbulkparam->tol						= 1e-6;
	sbulkparam->step_size				= 0.1;
	sbulkparam->iter_max				= 1000;
...
void fn_obt_translProjBox_matrix (stu_bulk_param	*sbulkparam)
{
	...
	for (int i = 0; i < sbulkparam->dimCpt; i++)
	{
		...
		for (int jj = 0; jj < sbulkparam->dimPhy; jj++)
			tpTmp.val[i] += sbulkparam->translVec.val[jj] * sbulkparam->projMat.val[jj][i];
	}
						...
						const	char		*key )
{
	...
	fprintf(fp, "\t# the plot range along y+direction; (y_range * 2*PI);\n");
	fprintf(fp, "z_range\t\t\t= %.15e", ssysparam->z_range);
	fprintf(fp, "\t# the plot range along z+direction for 3D phases; (z_range * 2*PI);\n");
...
void fn_stu_system_param_print (stu_system_param	*ssysparam)
{
	if ( ssysparam->print_level > 0 )
	{
		...
		printf("\t ---> The plot range along y-direction is [0, % .5e * 2*PI].\n", ssysparam->y_range);
		printf("\t ---> The plot range along z-direction is [0, % .5e * 2*PI].\n", ssysparam->z_range);
