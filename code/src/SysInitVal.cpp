							...
							stu_system_param	*ssysparam )
{
	...
	/* save results; */
	fn_save_cbnd ( ssysparam->scbndv->d0bnd, 0, xlen, cplxReDofs );
	if ( ssysparam->scbndv->isTest )
	{
		fn_save_cbnd ( ssysparam->scbndv->d2bnd, 2, xlen, cplxReDofs );
		if ( strcmp(model_type, "LP") == 0 )
		{
			fn_save_cbnd ( ssysparam->scbndv->d4bnd, 4, xlen, cplxReDofs );
			fn_save_cbnd ( ssysparam->scbndv->d6bnd, 6, xlen, cplxReDofs );
		}
	}
...
void fn_save_cbnd (	tvec<fftw_complex>	dsbnd,	int order,	int xlen, int cplxReDofs)
{
	...
	char fwFile[FILELEN];
	sprintf(fwFile, "%s/sys_d%dbnd.dat", rsltDir, order);
