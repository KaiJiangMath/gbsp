									...
									int					order )
{
	...
	for (int i = 0; i < cplxDofs; i++)
	{
		...
		for (int s = 1; s <= 2; s++)			// s=1 for left; s=2 for right;
		{
			 ...
			 * (R_x'PBk)^j * rhoCplx * exp(i(R_x'PBk)'x); j = 0,1,2,...
			 */
						...
						stu_GJP_var		*sGJPv)
{
	...
	for (int i = 0; i < cplxDofs; i++)
	{
		...
		if ( sbulkparam->sbndv->isTest )
		{
			char dirname[FILELEN], fname[FILELEN];
			FILE *fwPath;
			/* xInterp; */
			sprintf(dirname, "%s/xInterp_bulk%d", rsltDir, sbulkparam->sflag);
			mkdir(dirname, 0755);
			...
			fclose(fwPath);
			/* rhs; */
			sprintf(dirname, "%s/rhs_bulk%d", rsltDir, sbulkparam->sflag);
			mkdir(dirname, 0755);
			sprintf(fname, "%s/%d.dat", dirname, i);
			fwPath = fopen(fname, "w");
			for ( int j0 = 0; j0 < nd; j0++ )
	...
	{
		char fwFile[FILELEN];
		sprintf(fwFile, "%s/bulk%d_rhoJCplx.dat", rsltDir, sbulkparam->sflag);
