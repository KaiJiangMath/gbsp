...
extern char paraDir[FILELEN];		// the directory about parameters;
extern char rsltDir[FILELEN];		// the directory to save results;
...
{
	tmat<int>	 indKspace;			// k in Fourier space;	row = cplxDofs, col = dimCpt;
	tmat<double> projPlane;			// R'PBk;				row = cplxDofs, col = dimPhy;
	tvec<double> Gsquare;			// |R'PBk|^2;			len = cplxDofs;

	tvec<fftw_complex> rhoReal;		// rho in real space;						len = cplxDofs;
	tvec<fftw_complex> rhoCplx;		// rho in reciprocal space;					len = cplxDofs;
	tvec<fftw_complex> gradient;	// gradient part of the considering model;	len = cplxDofs;
	tvec<fftw_complex> fftw_Ctmp;	// temporal variables for FFTW;				len = cplxDofs;
	tvec<fftw_complex> fftw_Rtmp;	// temporal variables for FFTW;				len = cplxDofs;
	tvec<fftw_complex> cplxTmp;		// temporal variables for FFTW;				len = cplxDofs;
	tvec<fftw_complex> quadTerm;	// quadratic term in reciprocal space;		len = cplxDofs;
	tvec<fftw_complex> cubTerm;		// cubic term in reciprocal space;			len = cplxDofs;
	tvec<fftw_complex> quarTerm;	// quartic term in reciprocal space;		len = cplxDofs;
...
typedef struct stu_system_param
{
	...
	/* plotting parameters; */
	double	y_range;			// the plot range along y-direction; (y_range * 2*PI);
	double	z_range;			// the plot range along z-direction for 3D phases; (z_range * 2*PI);
	...
	tvec<int> NCpt;				// space grid of the Fourier direction;
